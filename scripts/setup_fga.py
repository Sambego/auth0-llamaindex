...
from openfga_sdk.models import (
    Metadata, ObjectRelation, RelationMetadata, RelationReference,
    TupleToUserset, TypeDefinition, Userset, Usersets, WriteAuthorizationModelRequest,
)
...
def parse_model_dsl(dsl: str) -> WriteAuthorizationModelRequest:
    ...
    return WriteAuthorizationModelRequest(schema_version=schema_version, type_definitions=type_defs)


async def main() -> None:
    config = fga_config()
    if not config.store_id:
        raise ValueError("FGA_STORE_ID is not set. Create a store at https://dashboard.fga.dev")
    ...
    async with OpenFgaClient(config) as fga:
        ...
        tuples = [
            ClientTuple(user=str(t["user"]), relation=str(t["relation"]), object=str(t["object"]))
    ...
    print("FGA setup complete. Run the app with: python main.py")
